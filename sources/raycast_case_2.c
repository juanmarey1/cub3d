...
static int	dist_y(t_cub *cub, int j, double y_distance)
{
	...
	cub->grid_raycast_y = 0;
	cub->sprite_direction = 'S';
	if (cub->angle == PI / 2)
		return (j - 1);
	cub->grid_raycast_x = cub->grid_raycast_x + (y_distance * cos(cub->angle));
	return (j - 1);
}
...
static int	dist_x(t_cub *cub, int i, double x_distance)
{
	...
	cub->grid_raycast_x = 100;
	cub->sprite_direction = 'E';
	if (cub->angle == PI)
		return (i - 1);
	cub->grid_raycast_y = cub->grid_raycast_y + (x_distance * sin(PI - cub->angle));
	return (i - 1);
}
