...
static int	dist_y(t_cub *cub, int j, double y_distance)
{
	...
	if (cub->angle == (3 * PI) / 2)
		return (j + 1);
	cub->grid_raycast_x = cub->grid_raycast_x - (y_distance * sin((3 * PI) / 2 - cub->angle));
	return (j + 1);
}
...
static int	dist_x(t_cub *cub, int i, double x_distance)
{
	...
	if (cub->angle == PI)
		return (i - 1);
	cub->grid_raycast_y = cub->grid_raycast_y - (x_distance * cos((3 * PI) / 2 - cub->angle));
	return (i - 1);
}
